...
impl Cpu {
    fn init() -> Self {
        return Cpu{X: 1, cycles: 0, CRT: vec![vec!['.'; 40]; 6]}
    }
    ...
    fn get_CRT(&self) -> String{
        self.CRT.iter().map(|line| line.into_iter().collect()).collect::<Vec<String>>().join("\n")
    }
}
...
fn parse_input(purp: Purpose) -> Vec<Instruction> {
    ...
    input.iter().map(|l| {
        ...
        match instr_str[0] {
            ...
            "addx" => Instruction::Addx{v: instr_str[1].parse::<i16>().unwrap()},
            &_ => todo!(),        
        }
    }).collect()
}
