    ...
    fn part_2(&mut self) -> String {
        ...
        for r in 1..n_rows-1 {
            for c in 1..n_cols-1 {
                ...
                for pos in (0..r).rev() {
                    left_distance += 1;
                    if *self.field.get((c,pos)).unwrap()>= el {break;}
                }
                ...
                for pos in (0..c).rev() {
                    up_distance += 1;
                    if *self.field.get((pos,r)).unwrap()>= el {break;}
                }
...
fn parse_input(purp: Purpose) -> DMatrix<u8> {
    ...
    let field :Vec<u8> = input.iter().map(|l| l.chars().map(|c| c.to_string().parse::<u8>().unwrap())).flatten().collect();
